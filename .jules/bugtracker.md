...
| BUG-004 | **Low** | Unsafe Integer Cast in `DirectHost::Send` | Calculation `(0u - (uint32_t)size)` relies on implicit casting behavior when handling negative `int32_t`. While strictly valid C++, it is brittle. | Use `std::abs` or explicit logical branching for clarity and safety assurance. | Approved (Fix with 007) | **Resolved** |
...
| BUG-007 | **High** | C++ UB on `INT_MIN` Negation | `DirectHost::ProcessGuestCalls` performs `-reqSize` on `INT_MIN` (`-2^31`), which is Undefined Behavior in C++. | Use `(0u - (uint32_t)reqSize)` for safe negation to handle 2's complement correctly. | Approved | **Resolved** |
| BUG-008 | **Medium** | FD Leak on Exec (Linux) | `sem_open` and `shm_open` lack `O_CLOEXEC`, causing file descriptors to leak to child processes if `exec` is called. | Add `O_CLOEXEC` flag to `open` calls in `go/platform_linux.go`. | Approved | **Resolved** |
