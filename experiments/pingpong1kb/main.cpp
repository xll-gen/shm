...
void cleanup(int num_threads) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        std::string h_name = "/pp1k_h_" + std::to_string(i);
        std::string g_name = "/pp1k_g_" + std::to_string(i);
...
void worker(int id, Packet* packet, int iterations, WorkerResult& result) {
    ...
    std::string g_name = "/pp1k_g_" + std::to_string(id);
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        for (int k = 0; k < 1024; ++k) {
            packet->data[k] = dist(rng);
            // Expected response: bitwise NOT of request
            expected_data[k] = ~packet->data[k];
        }
        packet->req_id = i;
        ...
        // Verify Data
        if (memcmp(packet->data, expected_data, 1024) != 0) {
            std::cerr << "Data mismatch in thread " << id << " at iteration " << i << std::endl;
            exit(1);
        }
...
int main(int argc, char* argv[]) {
    ...
    // Reduce iterations slightly as 1KB memcpy adds overhead, though for 100k it should be fine.
