...
func worker(id int, packet *Packet, wg *sync.WaitGroup) {
	...
	hName := C.CString(fmt.Sprintf("/pp_c_h_%d", id))
	gName := C.CString(fmt.Sprintf("/pp_c_g_%d", id))
	...
	for i := 0; i < 50; i++ {
		...
		if C.is_sem_failed(semHost) == 0 {
			C.sem_close(semHost)
		}
		if C.is_sem_failed(semGuest) == 0 {
			C.sem_close(semGuest)
		}
