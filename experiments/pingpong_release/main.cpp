...
void worker(int id, Packet* packet, int iterations, WorkerResult& result) {
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        // Signal request
        // Use release to ensure payload (val_a, val_b, req_id) is visible before the flag update.
        packet->state.store(STATE_REQ_READY, std::memory_order_release);

        // Wake guest if sleeping
        // We use seq_cst here for the sleep flag to prevent StoreLoad reordering that could lead to lost wakeups.
        ...
        for (int spin = 0; spin < spin_limit; ++spin) {
            // Use acquire to ensure we see the payload written by the guest after we see the state change.
        ...
        if (ready) {
            ...
            // 2. Sleep Phase
            // Use seq_cst to ensure "Store host_sleeping" happens before "Load state".
            packet->host_sleeping.store(1, std::memory_order_seq_cst);

            // Double check (Critical to avoid lost wakeups)
            // Use acquire (via the main state load logic) or seq_cst if strictly needed for the barrier.
            // Since we stored with seq_cst, a subsequent seq_cst load provides the barrier.
            if (packet->state.load(std::memory_order_seq_cst) == STATE_RESP_READY) {
                ...
                // Loop check can use acquire as we are just waiting for the signal now.
    ...
    packet->state.store(STATE_DONE, std::memory_order_release); // Signal done with release
