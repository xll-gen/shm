...
// Cleanup SHM and semaphores
void cleanup(int num_threads) {
    shm_unlink(SHM_NAME);
    for (int i = 0; i < num_threads; ++i) {
        std::string hName = "/pp_opt_h_" + std::to_string(i);
        std::string gName = "/pp_opt_g_" + std::to_string(i);
        sem_unlink(hName.c_str());
        sem_unlink(gName.c_str());
    }
}

void worker(int id, Packet* packet, int iterations, WorkerResult& result) {
    // Create per-thread semaphores
    std::string hName = "/pp_opt_h_" + std::to_string(id);
    std::string gName = "/pp_opt_g_" + std::to_string(id);

    // Unlink first to ensure fresh start
    sem_unlink(hName.c_str());
    sem_unlink(gName.c_str());

    sem_t* sem_host = sem_open(hName.c_str(), O_CREAT, 0666, 0);
    if (sem_host == SEM_FAILED) {
        perror("sem_open host");
        exit(1);
    }
    sem_t* sem_guest = sem_open(gName.c_str(), O_CREAT, 0666, 0);
...
int main(int argc, char* argv[]) {
    ...
    for(int i=0; i<num_threads; ++i) {
        // Ensure packets are spaced by 128 bytes.
        // Since Packet is sizeof(Packet) == 128, packets[i] handles the offset automatically.
        // Check alignment
        uintptr_t addr = (uintptr_t)&packets[i];
        if (addr % 128 != 0) {
            std::cerr << "Error: Packet " << i << " is not aligned to 128 bytes! Addr: " << addr << std::endl;
        }
