...
type Packet struct {
	...
	ValB          int64
	Sum           int64
	Pad           [88]byte // Matches common.h padding (128 - 40 = 88)
}
...
func worker(id int, packet *Packet, wg *sync.WaitGroup) {
	...
	// Use optimized semaphore names
	hName := C.CString(fmt.Sprintf("/pp_opt_h_%d", id))
	gName := C.CString(fmt.Sprintf("/pp_opt_g_%d", id))
...
func main() {
	...
	packetSize := uintptr(128) // Matches C++ Packet alignment
