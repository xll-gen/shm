...
The field therefore has one *negative* property, and it is normative: **`appMsgType` MUST NOT affect reassembly in any way.** Two `STREAM_START`s identical except for this field MUST drive byte-identical reassembly behavior — same acceptances, same rejections, same delivered bytes. Delivering the tag to the application (see "Receive-side delivery" below) does not weaken this and MUST NOT be allowed to: the tag is carried *alongside* the reassembled bytes, and is never consulted while producing them. (Tripwires: `go/stream_parity_table_test.go::TestStreamReassembly_AppMsgTypeIsInertToReassembly` and the matching check at the end of `tests/test_reassembler_parity_table.cpp`, which replay every parity case with the field set to garbage and compare digests. Both drive the *untyped* entry point, whose observable behavior the tag cannot touch.)
...
**Receive-side delivery: a PARALLEL, typed entry point (MUST).** Between `0x00080000` and this clause the field was write-only — senders wrote it, both reassemblers parsed past it, and nothing could read it. That gap is closed here, and it is **not** a wire change: no byte moves, no byte changes meaning, and `SHM_VERSION` is **NOT** bumped. Claiming the slot at the `0x00080000` bump is exactly what bought that; this clause spends it.

Each implementation MUST offer, *alongside* the existing completion callback, a second entry point whose completion callback additionally receives the stream's `appMsgType`:

- **Go** (`go/stream.go`): `type TypedStreamHandler func(streamID uint64, appMsgType uint32, data []byte)` and `NewStreamReassemblerTyped(onStream TypedStreamHandler, fallback ...)`.
- **C++** (`include/shm/StreamReassembler.h`): `using OnStreamTypedFn = std::function<void(uint64_t streamId, uint32_t appMsgType, const std::vector<uint8_t>& data)>` and the `StreamReassembler(OnStreamTypedFn, cfg)` constructor overload.

Four obligations bind that entry point, and the first is what makes the other three testable:

1. **Additive.** The existing `(streamId, data)` entry point (`NewStreamReassembler` / `StreamReassembler(OnStreamFn, cfg)`) MUST keep its signature and its behavior. The untyped form is defined as the typed form with the tag discarded, so the two can never diverge in reassembly behavior. **Go is additive without exception** (a new function name, no overloading). **C++ has ONE source-compatibility exception**, inherent to adding a constructor overload: an argument convertible to BOTH `OnStreamFn` and `OnStreamTypedFn` — `StreamReassembler r(nullptr)`, `r({})`, `r([](auto&&...){})` — became ambiguous, verified by compiling one TU against the pre-change header (clean) and the new one (three "call of overloaded … is ambiguous" errors, g++ 14.2 `-std=c++17`). No caller in this tree or any documented usage is affected, and the failure is always a COMPILE error, never a silent behavior change; the fix is to name the form (`OnStreamFn(nullptr)`). The exception is recorded on the `OnStreamFn` constructor in `include/shm/StreamReassembler.h`. Say "additive", not "purely additive", when restating this obligation.
2. **The tag is the one this stream's own `STREAM_START` carried** — captured into the reassembly context at `STREAM_START` and delivered from there. It is emphatically NOT "the most recently observed `appMsgType`": with several streams in flight, each completion MUST report its own stream's tag, whatever order the streams started and completed in. A `STREAM_START` that replaces a live context for the same `streamId` (§3.3.4) replaces its tag along with the rest of its state.
3. **Both completion paths deliver it.** The empty stream (`totalChunks == 0`, completed inside `STREAM_START` itself) and the ordinary chunk-driven completion MUST both pass the tag. A dropped or evicted stream delivers nothing at all, tag included.
4. **Unvalidated.** `0` is delivered as `0`; every other value is delivered verbatim. There is no rejection, no normalisation, and no default substitution — see the opacity MUST above.

Regression: `go/stream_apptype_delivery_test.go` and `tests/test_reassembler_appmsgtype_delivery.cpp` are one artifact in the sense of the §3.3.4 parity pair — the same four obligations asserted case for case on both sides. Their non-vacuity is the whole point: each side is confirmed by mutation (deliver a hard-coded constant instead of the parsed field), and each asserts a tag value that is distinct from every other field in the header, so a receiver that read `totalChunks@16` instead of `appMsgType@20` fails them too.
