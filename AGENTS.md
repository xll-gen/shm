...
* **MOSTLY DONE — v0.7.0 (lease) + v0.7.1 (reclamation API)**: Crash-time slot cleanup.
  ...
  * **REMAINING (later release):** (a) Auto-reclamation hook inside `WaitStrategy::Wait` so waiters self-heal without a separate watchdog goroutine. (b) End-to-end crash-process test that spawns a child guest, kills it mid-handler at random points, and verifies the host reclaims the slot. The current property test stresses CAS races on a single machine; the missing test layer is true process-kill injection.
