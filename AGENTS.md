...
* **IN DESIGN (v0.7.0 target):** Crash-time slot cleanup via heartbeat lease in `SlotHeader::reserved[36]` (carving 8 bytes for `atomic<uint64> lease`). Design documented in `SPECIFICATION.md §6 "Future: Crash-Time Slot Cleanup"`. Implementation deferred until a property-based crash-injection test exists, because picking the wrong heartbeat cadence risks reclaiming slots a slow peer is still using → data corruption.
