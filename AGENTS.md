...
* **MOSTLY DONE — v0.7.0 (lease) + v0.7.1 (reclamation API)**: Crash-time slot cleanup.
  ...
  * **REMAINING (v0.7.3 target):** End-to-end crash-process test that re-execs the test binary as a child "guest" which claims a slot then exits without releasing. Parent calls `TryReclaimAbandonedSlot` (or relies on auto-reclaim) and asserts recovery. Validates the real cross-process semaphore + shared memory lifecycle, not just the in-process CAS race that `TestTryReclaim_NoDoubleClaim_Property` covers.
