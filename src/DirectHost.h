...
public:
    ...
    bool Init(const std::string& shmName, uint32_t numQueues, uint32_t dataSize = 1024 * 1024) {
        ...
        // Better to zero out to avoid stale state
    ...
    void Shutdown() {
        ...
        // Send Shutdown signal to all slots
        for (auto& slot : slots) {
             // We can't easily force send if busy, but we can try
             // Or just let it be. Guest will likely timeout or see shared memory close.
             // Proper shutdown: Send MSG_ID_SHUTDOWN
             // For now, just clean up resources.
             // If we really want to shutdown guests, we should try to send MSG_ID_SHUTDOWN.
    ...
    // Blocking Send
    // Returns bytes read into response buffer, or -1 on error
    int Send(const uint8_t* data, uint32_t size, uint32_t msgId, std::vector<uint8_t>& outResp) {
            ...
            if (slot->header->state.load(std::memory_order_acquire) == SLOT_RESP_READY) {
                ...
                // Wait
                 while (slot->header->state.load(std::memory_order_acquire) != SLOT_RESP_READY) {
                    Platform::WaitEvent(slot->hRespEvent, 100); // 100ms timeout check
                 }
                 ready = true;
                 slot->header->hostState.store(HOST_STATE_ACTIVE, std::memory_order_relaxed);
            }
        ...
        // 6. Release Slot
        // Setting to FREE allows others to use it
