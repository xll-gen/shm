...
// TestGuestCallNegativeRespSize_New reproduces the bug where negative RespSize (end aligned data)
// is ignored by DirectGuest.SendGuestCall.
func TestGuestCallNegativeRespSize_New(t *testing.T) {
	shmName := "BugReproSHM_New"
    UnlinkShm(shmName)
    UnlinkEvent(fmt.Sprintf("%s_slot_0", shmName))
    UnlinkEvent(fmt.Sprintf("%s_slot_0_resp", shmName))
    UnlinkEvent(fmt.Sprintf("%s_guest_call", shmName))
    ...
    go func() {
        ...
        respLen := len(respData)
        dataBase := slotHeaderPtr + 128
        // RespOffset 512. MaxResp 512.
        // End of slot: dataBase + 1024.
        destPtr := dataBase + 1024 - uintptr(respLen)
...
// TestNoGuestSlots verifies that SendGuestCall fails gracefully
// when the Host is initialized with NumGuestSlots = 0.
func TestNoGuestSlots(t *testing.T) {
	shmName := "NoGuestSlotsSHM"
    UnlinkShm(shmName)
    UnlinkEvent(fmt.Sprintf("%s_slot_0", shmName))
    UnlinkEvent(fmt.Sprintf("%s_slot_0_resp", shmName))
    ...
    exHeader.Version = Version
    exHeader.NumSlots = 1
    exHeader.NumGuestSlots = 0 // Key: 0 Guest Slots
    exHeader.SlotSize = 1024
    exHeader.ReqOffset = 0
    exHeader.RespOffset = 512

    // Create Events for Host Slot 0 (Required for NewDirectGuest to pass)
    reqEvName0 := fmt.Sprintf("%s_slot_0", shmName)
    respEvName0 := fmt.Sprintf("%s_slot_0_resp", shmName)
    hReq0, _ := CreateEvent(reqEvName0)
    hResp0, _ := CreateEvent(respEvName0)
    ...
    // 3. Attempt SendGuestCall
    _, err = guest.SendGuestCall([]byte("Test"), MsgTypeGuestCall)
    if err == nil {
        t.Fatalf("SendGuestCall should have failed")
    }

    // 4. Verify Error Message
    expectedErr := "no guest slots available"
